...
import { deleteInterviewDocument } from '../utils/documentUtils';
...
export default function PrepInterview() {
  ...
  const deleteDocument = async (documentId) => {
    ...
    const result = await deleteInterviewDocument(selectedInterview.id, documentId);
                  ...
                  {selectedInterview.documents.map((doc) => (
                    ...
                    >
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '0.5rem' }}>
                        <h4 style={{ fontSize: '0.95rem', fontWeight: '600', margin: 0, color: '#1f2937', flex: 1 }}>
                          {doc.title}
                        </h4>
